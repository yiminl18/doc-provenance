...
import json, nltk, os, re, sys, tempfile, time
...
# In pdf_utils.py - Update to handle file paths
def is_pdf_text_extractable(file_path_or_content, min_text_length=100):
    """Check if PDF has extractable text using pdfminer"""
    temp_path = None
    try:
        # Handle both file paths and bytes
        if isinstance(file_path_or_content, str):
            # It's a file path
            text = extract_text(file_path_or_content)
        else:
            # It's bytes - create temp file
...
def estimate_pdf_size_from_pages(page_count):
    """Rough estimate of PDF size based on page count"""
    # Very rough estimate: 100KB per page for text PDFs
    return page_count * 100 * 1024 if page_count else 0

def list_extractable_pdfs(drive_service, folder_id=None):
    """List PDFs that have extractable text"""
    ...
    for file in results.get('files', []):
        # Download and check if text is extractable
        file_content = drive_service.files().get_media(fileId=file['id']).execute()
        
        if is_pdf_text_extractable(file_content):
            extractable_pdfs.append({
                'id': file['id'],
                'name': file['name'],
